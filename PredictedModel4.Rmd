...
Each of these work well with category based outcomes.   Each model utilizes the same training set, and has the same seed set so that validation of accuracy can be performed.  
...
```{r echo=FALSE}
#This allows for the loading of the models from files.  They were saved when the build[model]() function was ran.   This allows time to build better documentation.
...
I utilize several different methods to improve performance including the parallel computing capabilities that have been developed in the Revolutions analytics version of R.  Below is a table that shows the difference in execution on a MacBook Pro which has 8 logical cores when parallel processing is utilized.  This has a large impact on the ability to test and create  Additional information can be found in the Appendix: Parallel Processing.
...
Cross validation can be performed using three separate methods.     
...
##Confusion Matricies - Error by class    
###GBM Model Prediction Validation
```{r modelGbm, echo= FALSE, eval=TRUE,message=FALSE}
validatePredictionModel(gbmFit,validation)
```

###SVM Model Prediction Validation
...
###Random Forest Model Prediction Validation
...
###Error Rate
The error rates for each model are shown above based on the validation set.  When looking at the current models, we see an accuracy rate above 99% for rf using the validation set.   We utilize the 3 models to reduce the error rate more.  This is performed by using a voting model to combine the outcomes.

##Combined Model - Voting
To get value of the three different models, I have ensembled the models, and utilized the majority vote across the models.  The outcome variable was created by choosing the majority of the 3 predicted variables (classe), and choosing the factor with max weighting x count.  This model works well when there is less data.  The code is presented in the "Appendix:Code voteForOutcome()"


###Weighting   
Weighting is calculated using the _Balanced Accuracy_ from the confusition matrix which is comparing the outcomes of each model to the actual outcome of the validation set. The weights are modfied to ensure that a .5 error rate is significantly lower than .9999 by using an exponential function.

```{r setWeighting,results="asis",message=FALSE,echo=FALSE}
library(tidyr)
library(knitr)
...
predicted.validation.results  <- predictAll(gbmFit,svmFit,rfmFit,data=validation)
expected.outcome <- validation$classe
weights <- weightsOf(data = predicted.validation.results ,actual = expected.outcome)
kable(weights,digits = 3)
```

###Test Results   
The results of the prediction using the testing data has acheived 100% accuracy.   I would not expect this to be true with a larger data set.
```{r createTestResults,results="asis",message=FALSE,echo=FALSE}
predicted.test.results  <- predictAll(gbmFit,svmFit,rfmFit,data=testing)
test.set.results <- voteForOutcome(predicted.test.results,weights = weights)
...
---
#Appendix: Code
I have embedded the code in the appendix so that it is easily visible to the reviewers, and helps to create reproducibility.   
...
##buildData()
This function is purpose is to load the training and testing data.  Training data is split into 2 sets.  75% of the observations are used for model training, and 15% used for validation using the function caret::createDataPartition()
The training and validation data is writen into separate files "./data/training.csv" and "./data/testing.csv" for reuse.   An additional feature is based on the parameter "quickRun".  If this parameter is set to TRUE, then it takes a small sample of the data set, and utilizes this as the learning and validation. 
...
##sendStartMessage()
Send out the email message at the beginning of the model creation
```{r code=capture.output(dump('sendStartMessage', '')),tidy=TRUE}
#Replaced with Function
``` 

##sendEndMessage()
Send out the email message at the end of the model creation.
...
``` 
#Voting 
I utilized several different models to predict, and built a function for ensembling the outcomes through voting.  The voting method weights the votes based on the class and _Balanced Accuracy_ determined by using the confusion matrix.   This is performed in the following code.   

##voteForOutcome()    
This is the code that performs the vote based on weighting of outcomes.
```{r code=capture.output(dump('voteForOutcome', '')),tidy=TRUE}
#Replaced with Function
``` 

##predictAll(...,data)   
This function takes a set of models as the first arguments in the dots, and the data that it will predict against.  It allows multiple models to be easily processed with the same data set. 

```{r code=capture.output(dump('predictAll', '')),tidy=TRUE}
#Replaced with Function
```
##weightsOf()
This is the code that performs the weights the outcomes based on confusion matrix _Balanced Accuracy_.  This could be improved in the future by showing the expected accuracy.
```{r code=capture.output(dump('weightsOf', '')),tidy=TRUE}
#Replaced with Function
``` 
\newpage
