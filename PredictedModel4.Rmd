...
###Error Rate
The error rates for each model are shown above based on the validation set.  When looking at the current models, we see an accuracy rate above 99% for rf.  We utilize the 3 models to reduce the error rate.  This is performed by using a voting model to combine the outcomes.
